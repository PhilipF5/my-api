...
module.exports = async function(context, req) {
	...
	let lastUpdate = await db.collection("cacheUpdates").find({ type: "blogPosts" }).sort("time", -1).limit(1).toArray();
	...
	let documents = await db.collection("blogPosts").find().toArray();
	context.res = {
		body: documents,
	};
};
...
async function refresh(db) {
	...
	let feed = await request.get("https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@philipf5", { json: true });
	await Promise.all([
